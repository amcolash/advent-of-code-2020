...
const input = fs.readFileSync(path.join(__dirname, 'input3.txt')).toString().trim();
...
// From https://stackoverflow.com/a/61352745/2303432
function findNthOccurence(string, nth, char) {
  let index = 0;
...
function generateAddresses(a) {
  ...
  console.log(a.toString(2).padStart(36, '0'));
  console.log(mask);

  const numFloating = (mask.match(/X/g) || []).length;
  const variations = Math.pow(2, numFloating);
  const addresses = new BigUint64Array(variations);

  for (let i = 0; i < variations; i++) {
    let tempA = maskedA;

    for (let j = 0; j < numFloating; j++) {
      const floatingIndex = mask.length - findNthOccurence(mask, j + 1, 'X') - 1;

      // if bit is set to 1
      if ((i >> j) & 1) {
        tempA |= BigInt(1 << floatingIndex);
      } else {
        tempA &= BigInt(~(1 << floatingIndex));
      }
    }
